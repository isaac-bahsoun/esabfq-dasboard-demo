...
import { SearchInput, TableWrapper } from "./UnassignedTrainingTableStyle.js";
import LoadingScreen from "../../elements/Loading.jsx";
import { getAllCandidatesUnassignedTraining } from "../../apiCalls/candidate.js";


export default function UnassignedTrainingTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getAllCandidatesUnassignedTraining();
        setCandidates(data);
            ...
            <th>Created At</th>
                ...
                <tbody>
                  {filteredCandidates.map((item) => (
                    ...
                    <td>{item.qualifications_applied_for}</td>
                    <td>{new Date(item.application_created_at).toLocaleDateString('en-GB')}</td>
                  </tr>  
                  ))}
                </tbody>
              </table>
